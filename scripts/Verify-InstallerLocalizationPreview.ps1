...
foreach($requiredText in @('Supported OS for this release: Windows 11 x64.','Windows 10 is not supported by the current pinned upstream driver package.','This installation includes:','Safety boundaries:','Uninstall options:','Driver removal is fail-closed:')){
    if(-not $infoEnText.Contains($requiredText)){ throw "English InfoBefore section missing: $requiredText" }
}
foreach($requiredText in @('当前版本仅支持 Windows 11 x64。','当前固定的上游驱动包不支持 Windows 10。','本次安装包括：','安全边界：','卸载选项：','驱动移除采用失败即保留策略：')){
...
Write-Host "[PASS] Bilingual installer UX/safety baseline remains frozen; stable minimum OS gate is Windows 11 x64 build 22000."
