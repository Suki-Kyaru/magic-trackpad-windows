...
一个面向 Windows x64 的 Apple Magic Trackpad 安装、诊断与生命周期管理项目。
项目基于
...
> 代码签名，内嵌上游驱动继续保持原始 Microsoft 签名。当前 `v0.1.0-rc.1` 源码属于
...
Windows 10 验证仍在进行中；最低安装门槛本身并不代表已经形成项目级正式支持声明。
...
基线。当前 `v0.1.0-rc.1` 属于正式版候选源码，必须完成 Windows 10 / Windows 11
正式版验证后才能作为稳定版公开发布。
...
当前源码已经升级到 `0.1.0-dev.6.0`，根目录 `VERSION` 与
`installer/setup.iss` 中的 `#define MyAppVersion` 已保持一致。应在干净、已提交的
源码状态下构建预发布候选：
...
`Build-Installer.ps1` 仍会在 `VERSION` 被设为已经冻结的
`0.1.0-dev.5.4.2` 时主动阻断；`dev.6.0` 只有在 VERSION/Inno 版本一致时才允许
构建。真正编译前仍会重新执行驱动载荷、安装器、卸载和许可证分发门禁。
